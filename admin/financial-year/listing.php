        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header d-flex justify-content-end align-items-center gap-2">
              ...
              <button type="button" class="btn btn-success btn-sm mx-2" onclick="activateSelectedYears()">Active</button>


              <button type="button" class="btn btn-danger btn-sm" onclick="blockSelectedYears()">Block</button>
              <button type="button" class="btn btn-danger btn-sm mx-2" style="background-color:#FD7E14;" onclick="deleteSelectedYears()">Delete</button>
            </div>
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    <th class="position-relative"># <input type="checkbox" class="all-years-checkbox position-absolute" style="top:50%;left:50%;" onchange="selectAllYears(this)"></th>
                    <th>Sl. No.</th>
                    <th>Start Year</th>
                    <th>End Year</th>
                ...
                <tbody>
                  <?php foreach ($years as $index => $year) : ?>
                    <tr>
                      <td class="position-relative px-4"><input type="checkbox" class="year-checkbox position-absolute" style="top:50%;left:50%;" value="<?php echo $year['id']; ?>"></td>
                      <td><a href="<?=
                                    "
                        http://localhost/xampp/MARS/appolopublicschool.com/admin/financial-year/edit.php?id={$year['id']}"
                                    ?>"><?php echo $index + 1; ?></a></td>
                      <td><?php echo $year['start_year']; ?></td>
                      <td><?php echo $year['end_year']; ?></td>
                      <td><?php echo $year['fiscal_year']; ?></td>
                      <td><?php echo $year['status'] == 1 ? 'Active' : 'Blocked'; ?></td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
                <tfoot>
